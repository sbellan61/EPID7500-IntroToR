...
## You'll notice "?" in several places throughout this script, indicating places where you must edit the code
...
## use sample() to create a smoker variable for each individual that has a 50% chance of being T or F
myStudy <- myStudy %>%
  mutate(smoker = sample(?))
...
lifetimeLungCancerRisk <- crossing(???)
...
lifetimeLungCancerRisk <- lifetimeLungCancerRisk %>%
  mutate(risk = ??? ) ## risk amongst Canadians from above reference, matched to sex/smoker combinations
