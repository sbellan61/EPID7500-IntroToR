...
group_by(lifetimeLungCancerRisk, sex) %>% 
    summarize(truePR = risk[smoker==T]/risk[smoker==F])

####################################################################################################
## 2. Create a new table where the risk ratio is only 1.5
lifetimeLungCancerRisk1.5 <- group_by(lifetimeLungCancerRisk, sex) %>%
    mutate(risk = replace(risk, smoker==F, risk[smoker==T]/1.5))
lifetimeLungCancerRisk1.5

## 3. Run the study simulator with this risk
myStudy <- createMyStudy(sampSize=300, printXtabs=T, groupRisks = lifetimeLungCancerRisk1.5)
print(actualPrevRatio <- CalcPrevRatio(myStudy))
permutationTestResults <- permuteStat(myStudy, browse=F)
calcPvalue(permutationTestResults)
...
for(i in seq_along(pValues)) {
    myStudy <- createMyStudy(sampSize = 200, groupRisks=lifetimeLungCancerRisk1.5)
    permutationTestResults <- permuteStat(myStudy, showHist=F)
    pValues[i] <- calcPvalue(permutationTestResults)
}
...
## this distribution
curve(function(x) dnorm(x, mean = 0, sd = 1), from = -3, to = 3)
