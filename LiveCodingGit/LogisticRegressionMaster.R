...
logOR_BMI_CI <- confint(mod1)['BMI',] ## Profile confidence intervals *Slower
logOR_BMI_CI <- confint.default(mod1)['BMI',] ## Wald confidence intervals *FASTER
...
## 9) Change your study simulation function so it returns a tibble with columns for the OR_BMI
## estimate, confidence intervals, and p value

simStudy <- function(sampSize=50000,
                     hazards = tb_risk_by_BMI, refHazard = 291/10^5, 
                     meanlogBMI=log(23.5), sdlogBMI=.3,
                     showXtabs=F, doMod=T, browse=F) {
    if(browse) browser()
    tb_risk_by_BMI <- tb_risk_by_BMI %>% mutate(tbhazard = refHazard * aHR)
    dat <- tibble(id=1:sampSize, BMI=rlnorm(sampSize, meanlogBMI, sdlogBMI)) %>%
        mutate(BMIcat = cut(BMI, BMIbreaks))
    ...
    if(doMod)
    {
        ...
        OR_BMI_CI <- exp(confint.default(tempmod)['BMI',])
...
temp <- simStudy(50000, doMod=T)
temp$OR_BMI

## 10) Create a function that does many runs of the above function, figures out if the CI on the BMI
## effect excludes the null hypothesis of OR=1, and then figures out the proportion of runs that
## that happens, returning it as power.

doManyRuns <- function(runs = 10,
                       argsForSimStudy=list(), browse=F) {
    if(browse) browser()
    for(jj in 1:runs) {
        temp <- do.call(simStudy, args=argsForSimStudy)
        if(jj==1) output <- temp$OR_BMI
        if(jj>1) output <- rbind(output, temp$OR_BMI)
    }
    output <- output %>%
        mutate(CIexcludesNull = (lower-1)*(upper-1) >0)
    power <- mean(output$CIexcludesNull)
    return(list(output=output, power=power))
    }

powerOut <- doManyRuns(argsForSimStudy=list(sampSize=5000, browse=F))
powerOut

## Do a power analysis across the following 5 sample sizes using a for loop
sampSizes <- c(500, 1000, 5000, 10^4, 5*10^4)
nss <- length(sampSizes)
powers <- rep(NA, nss)
for(ii in 1:nss) {
    powers[ii] <- doManyRuns(argsForSimStudy=list(sampSize=sampSizes[ii], browse=F))$power
}
powerDat <- tibble(sampSizes, powers)

ggplot(powerDat, aes(sampSizes, powers)) + geom_line() + xlab('sample size') + ylab('power')
